<template>
    ...
    <div style="margin-top: 20px" v-resize="resize" ref="tableBox">
                ...
                >
                    ...
                    <template v-else-if="item.actives" v-slot="scope">
                        <template v-for="active in item.actives">
                            <template
                                v-if="
                                    showButtom(active.perCode, scope.row) &&
                                    (active.show == undefined || active.show)
                                "
                            >
                                <template
                                    v-if="
        ...
        <div v-if="hidden">
            ...
            <template v-if="!pageShow">
                <div class="total">共{{ tableData.length }}条</div>
            </template>
        </div>
    </div>
</template>
...
export default {
    ...
    setup(props, context) {
        ...
        // 根据后台返回权限显示按钮
        const showButtom = (perCode, row) => {
            if (!perCode) {
                return true;
            } else {
                if (backType(perCode, "Function")) {
                    let code = perCode(row);
                    return isShowButtom(code);
                }
        ...
        onMounted(() => {
            getData({ ...state.listQuery, ...props.params });
        });
        // 获取头部高度
            ...
            if (props.headHight) {
                ...
                if (tableBox.value) {
                    ...
                    if (table__body) {
                        ...
                        state.topHead =
                            windowHeight - table__body - bottomSize - 30;
                    } else {
                        state.topHead = 630;
                    }
                }
        ...
        // 重置表格 刷新
        const reset = () => {
            getData({ ...props.params, ...state.listQuery });
        };
        // 更新表格
        const update = (params) => {
            state.searchParams = params;
            clearSelect();
            getData({ ...props.params, ...state.listQuery, ...params });
        };
        // 清空选项
