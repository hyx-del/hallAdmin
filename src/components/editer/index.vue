<template>
    <!--富文本editer-->
    <div v-show="show" :style="inStyle" @mouseleave="leave">
        <div class="editer" :id="code"></div>
    </div>
</template>
...
export default {
    ...
    setup(props, context) {
        ...
        const initEditer = () => {
            ...
            state.editerObj.config.height = 725;
            ...
            state.editerObj.config.onblur = function (newHtml) {
            ...
            state.editerObj.config.pasteTextHandle = (pasteStr) => {
                ...
                imgHtml &&
                    imgHtml.map((img) => {
                        ...
                        let u = src[1].split("?");
                        if (
                            !u[0].includes(url.OSSUrl) &&
                            !u[0].includes(url.IMGCDN)
                        ) {
        ...
        //
        const leave = () => {
            let modelValue = state.editerObj.txt.html();
