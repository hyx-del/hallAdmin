 ...
 * @LastEditTime: 2021-11-23 13:17:36
 * @Descripttion:  图片剪切
-->
    ...
    <div>
        <div class="lt_upload_img_list">
            ...
            <div
                ...
                class="
                    lt_cropper_input lt_cropper_upload_item
            ...
            <el-tooltip
                effect="light"
                :content="
                    tips || `请上传不超过${size}KB的JPEG、JPG、PNG格式的图片`
                "
                placement="bottom-start"
            >
                <div class="tips">
                    {{
                        ` ${fileList.length}/${limit},上传比例${ratioNumber},建议尺寸${ratio}`
                    }}
                </div>
            </el-tooltip>
        </div>
    ...
    setup(props, ctx) {
        ...
        const ratioNumber = ref(
            computed(() => {
                ...
                return (
                    parseInt(Number(Number(ratioArr[1] / ratioArr[0])) * 100) /
                        100 || 1
                );
            })
        );
        const state = reactive({
            ...
            parentVal: props.modelValue, // 同步父节点传递进来的 默认数据
            fileList: [], // 存储图片数据列表
            parameters: {
