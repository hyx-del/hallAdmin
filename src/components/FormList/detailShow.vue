                ...
                <div class="listShowValue" :style="{ ...inConfig.rightStyle }">
                    <div class="span" v-if="spanArr.includes(item.el)">
                        <div v-if="item.type == 'Array'">
                            <div
                                v-for="(i, index) in inFormObj[item.code]"
                                :key="index"
                            >
                                {{ i[item.labelCode] || i }}
                            </div>
            ...
            state.inList = state.inList.map((value) => {
                ...
                if (
                    ...
                    (!state.inFormObj[code] ||
                        state.inFormObj[code].length == 0)
                ) {
                    let spliteArr = backType(value.defaultValueSort, "String")
                        ? value.defaultValueSort.split(",")
                        : value.defaultValueSort;
                    spliteArr.some((v) => {
                ...
                if (el == "editer") {
                    try {
                        ...
                        state.inFormObj[code] = value.default || "未填写";
                    }
                }
                let rex = /[\u4e00-\u9fa5]+/gi;
                let long;
