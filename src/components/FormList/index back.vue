  ...
  setup(props, context) {
    ...
    let start = () => {
      state.inItemList = state.inItemList.map(item => {
        item = { ...state.inConfig.default, ...item };
      ...
      state.inList = state.inList.map(value => {
        ...
        if (value.default) {
          state.defaultValue[code] = value.default;
        }
    ...
    const formChange = (el, code, obj = {}) => {
      ...
      if (obj.allList && item.countFun) {
        state.otherCount[code] =
          obj.allList.length > 0 ? item.countFun(obj.allList) : item.default;
      }
      // 自动计算的值的监听
