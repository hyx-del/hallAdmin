...
// 表单验证
export const formCheck = (value, state, inFormObj) => {
    let rules = {};
    ...
    if (inputArr.includes(el)) {
        ...
        rules[code].push(min);
        // 有检查类型
        if (value.checkType && checkTypeArr.includes(value.checkType)) {
            let check = (rule, val, callback) => {
                if (val && !checkType[value.checkType].regular.test(val)) {
                    callback(
                        new Error(
                            value.typeError ||
                                checkType[value.checkType].error ||
                                value.placeholder
                        )
                    );
                }
    ...
    // 身份证检查
    if (value.checkFun == "idcard"||value.checkFun == "idCard") {
    ...
    // 自定义正则方法
    if (backType(value.checkFun, "Function")) {
        let checkFun = (rule, val, callback) => {
            if (val == "" || val == undefined) callback();
            let checkVal = value.checkFun(val, state.inFormObj);
            console.log('-------------')
            if (checkVal) {
                callback(new Error(`${checkVal}`));
            }
            callback();
        };
        let check = {
    ...
    if (value.lessCode != undefined) {
        let check = (rule, v, callback) => {
            if (v == "" || v == undefined) callback();
    ...
    if (el == "radio" || el == "checkbox") {
        let check = (rule, val, callback) => {
            console.log('+++++>',val,state.inFormObj[code],inFormObj[code])
            if (val == "" || (val == undefined && required)) {
                callback(new Error(value.error || value.placeholder));
            }
            callback();
        };
        rules[code].push({ validator: check, trigger: ['blur']});
        console.log(code,rules[code])
    }
    // 自定义验证的方法
    if (value.validator) {
    ...
    if (el == "date" || el == "datePicker") {
        ...
        if (value.maxTime || value.minTime) {
            let maxTime = new Date(
                value.maxTime || "2200-12-31 00:00:00"
            ).getTime();
            let minTime =
