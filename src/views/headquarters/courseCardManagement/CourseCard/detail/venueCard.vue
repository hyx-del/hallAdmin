<!--
 * @Author: 刘帝君
 * @Date: 2022-01-28 15:42:16
 * @LastEditors: 刘帝君
 * @LastEditTime: 2022-02-09 14:40:15
 * @Descripttion: 课程管理卡详情/期限卡-城市、场馆
-->
<template>
    <div>
        <el-row>
...
export default {
    setup() {
        const state = reactive({
            ...
            clickStore: (val) => {
                state.cityShops = val;
                state.isShowId = val.hallId ? "2" : "1";
                venueDetailAPI({
                    courseCardId: state.id,
                    hallId: state.isShowId == "1" ? null : val.hallId,
                    cityId: state.isShowId == "2" ? null : val.cityId,
                }).then((res) => {
                    const { code, data } = res;
            ...
            handleCheckChange: (data, checkedData) => {
                state.checkedData = checkedData.checkedKeys;
            },
        });
        onMounted(() => {
            // 侧边栏
            venueListAreaAPI().then((res) => {
                ...
                state.dataList.forEach((e) => {
                    e.id = e.cityId;
                    e.hallName = e.cityName;
                    e.halls.forEach((v) => {
                        v.id = v.hallId;
                    });
                });
                console.log(state.dataList, "侧边栏");
            });
            ...
            courseQueryAPI({
                ...
                const { code, data } = res;
                state.defaultData = data.hallId;
                console.log(data, "获取查询可用场馆");
            });
        });
