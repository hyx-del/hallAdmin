 ...
 * @LastEditTime: 2022-02-18 15:56:47
 * @Descripttion: 开卡申请订单
-->
<template>
   ...
   <div v-if="formDatas">
         ...
         <template #market> 
            <div class="dialog">
               <el-button type="primary"
                          size="mini"
                          @click="dialogVisibleSellFn">修改销售</el-button>
               <el-dialog v-model="dialogVisibleSell"
                          title="修改销售"
                          width="627px">
                  销售员：<el-select v-model="sell"
                             placeholder="请选择销售"
                             size="mini"
                             :filterable="true">
                     <el-option v-for="i in testSelect"
                                :key="i.value"
                                :label="i.name"
                                :value="i.id">
                     </el-option>
                  ...
                  <template #footer>
                     <span class="dialog-footer">
                        <el-button @click="dialogVisibleSell = false">取消</el-button>
                        <el-button type="primary"
                                   @click="sellSubmit()">确定</el-button>
                     </span>
                  </template>
               </el-dialog>
               <el-button type="primary"
                          size="mini"
                          @click="dialogVisible=true">修改记录</el-button>
               <el-dialog v-model="dialogVisible"
                  ...
                  <Table :showSortIndex="{ show: true, lable: '序号' }"
                         ...
                         :column="column"
                         v-if="dialogVisible"
                         :params="{orderId :queryObj.id,type:5}"
                         :hidden="false"></Table>
               </el-dialog>
            </div>
         </template>
      ...
      <div class="contract">
         ...
         <span class="tipsContract"
               v-if="(contract.isElec||contract.isPaper)">存在没有作废的{{contract.isElec?'电子':'纸质'}}合同 作废后可切换</span>
         ...
         <div v-if="contract.radio==0"
            ...
            <div v-for="(item,index) in contract.elec"
               ...
               <div v-if="item.status!=-1">
                  <div id="qrcode"
                       v-if="item.status==0">{{creatQrcode(item.signUrl)}}</div>
                  <!-- <img :src="item.signUrl"
                       v-if="item.status!=2"> -->
                  <el-button type="primary"
                             size="mini"
                             @click="print(item.id)">打印</el-button>
                  <el-button type="primary"
                             size="mini"
                             @click="destory(item.id)">作废电子合同</el-button>
                  <el-button type="primary"
                             size="mini"
                             @click="contractFn">刷新</el-button>
                  <p v-if="item.status==0">微信扫描二维码签署合同</p>

                  <!-- 卡规格为自定义的合同则需要审核 -->
                  <p style=" transform:translateX(-139px)"><b>签署状态：</b><span>{{contractStatus(item.status)}}</span>
                     <el-button size="mini"
                                v-if="item.status==1&&item.filed==0"
            ...
            <el-button size="mini"
                       @click="contractModel(1)"
                       v-if="!contract.isElec"
                       class="template">生成电子合同模板</el-button>
         </div>
         ...
         <div v-else
            ...
            <div v-for="(item,index) in contract.paper"
               ...
               <div v-if="item.status!=-1">
                  ...
                  <el-button type="primary"
                             size="mini"
                             @click="destory(item.id)">作废纸质合同</el-button>
                  <el-button type="primary"
                             size="mini"
                             @click="contractFn">刷新</el-button>
            ...
            <el-button size="mini"
                       @click="contractModel(2)"
                       v-if="!contract.isPaper"
                       class="template">生成纸质合同模板</el-button>
         </div>

         <!-- 模板弹框 -->
         <el-dialog v-model="dialogVisibleTemplate"
                    :title="`生成${type==1?'电子':'纸质'}合同`"
                    width="984px">
            <Form :formDatas="formDatasTemplate"
                  :formList="formListTemplate"
               ...
               <template #cancel>
                  <el-button size="mini"
                             @click="dialogVisibleTemplate=false">取消</el-button>
               </template>
            </Form>
         </el-dialog>
         ...
         <p style="display:flex"><b class="required">合同附件：</b>
            ...
            <el-button size="mini"
                       type="warning"
                       class="appendixesUpdateFn"
                       @click="appendixesUpdateFn">更新附件</el-button>
         </p>
         ...
         <div class="typePay"
            ...
            <!-- 支付修改 -->
            <div class="amend"
                 v-if="formDatas.orderStatus==20">
               <el-button size="mini"
                          @click="dialogVisiblePay=true">修改</el-button>
               <el-dialog v-model="dialogVisiblePay"
                          title="修改支付方式"
                          width="1112px">
                  <payType :formDatas="formDatasTypePay"
                           :dialog="true"
                           ref="formDatasTypePayRefDialog"
                           v-if="dialogVisiblePay"></payType>
                  <template #footer>
                     <span class="dialog-footer">
                        <el-button @click="dialogVisiblePay = false">取消</el-button>
                        <el-button type="primary"
                                   @click="paySubmit()">确定</el-button>
                     </span>
                  </template>
               </el-dialog>

               <el-button size="mini"
                          @click="dialogVisibleLog=true">修改记录</el-button>
               <el-dialog v-model="dialogVisibleLog"
                          title="修改记录"
                          width="1400px">
                  <Table :showSortIndex="{ show: true, lable: '序号' }"
                         :loadData="getListPay"
...
<script>
   import {
      computed, defineComponent, nextTick, onBeforeUnmount, onMounted, reactive, ref, toRefs, watch,
   } from 'vue';
   ...
   export default defineComponent({
      ...
      setup(props, context) {
         ...
         const state = reactive({
            ...
            */
            formList: [
               ...
               { el: "date", label: "生效时间", code: "startDate", width: "261px", disabled: true },
            ...
            testSelect: [],
            column: [
               { prop: 'oldSalesmanName', label: "修改前", html: (val) => { return val.changeBefore.name } },
               { prop: 'newSalesmanName', label: "修改后", html: (val) => { return val.changeAfter.name } },
               { prop: 'operatorName', label: "操作人", },
               { prop: 'updatedAt', label: "操作时间", },
            ],
            columnPay: [
               {
                  prop: 'oldSalesmanName', label: "修改前", align: 'left',
                  html: (val) => {
                     let { changeBefore } = val;
                     if (changeBefore) return tableStr(changeBefore);
                  }
               },
               {
                  prop: 'newSalesmanName', label: "修改后", align: 'left',
                  html: (val) => {
                     let { changeAfter } = val;
                     if (changeAfter) return tableStr(changeAfter);
                  }
               },
               { prop: 'operatorName', label: "操作人", width: 120 },
               { prop: 'updatedAt', label: "操作时间", width: 160 },
            ],
            formDatasTemplate: { getMsg: 0 },
            formListTemplate: [
               { label: "真实姓名", code: "memberName", required: true, rules: "zh" },
               { label: "证件号码", code: "idCard", required: true, rulesFn: "identity" },
               {
         ...
         const detail = async () => {
            ...
            courseOrdesDetail(queryObj).then(res => {
               if (res.code == 200) {
                  ...
                  if (orderStatus == 20 || (orderStatus == 10 && actualPrice > 0))
         ...
         //打印合同
         const print = (id) => {
            let src = `/admin/contract/open/print-contract?contractId=${id}`;
            printJs({ printable: src, type: 'pdf', });
         };

         //作废合同
         const destory = (id) => {
            destoryContract({ contractId: id }).then(res => {
               res.code == 200 ?
                  (contractFn(),
                     state.appendixes = null,
         ...
         //审核合同
         const checkContractFn = (id) => {
            checkContract({ contractId: id }).then(res => {
               res.code == 200 ?
                  (contractFn(), ElMessage.success("审核成功")) :
         ...
         const TemplateSubmit = (event) => {
            ...
            generateContracts(event).then(res => {
               if (res.code == 200) {
                  (contractFn(), ElMessage.success("生成模板成功"));
         ...
         //销售弹框修改
         const sellSubmit = () => {
            let { memberCourseCardId, orderStatus, salesmanId } = state.formDatas;
         ...
         const paySubmit = () => {
            formDatasTypePayRefDialog.value.formDataInfoPayType.scrollIntoViewFn('onSubmit', (formDate) => {
               ...
               let isChannel = regGroup(formDate);
               if (!isChannel) return ElMessage.error("支付方式不能为空");
               let isAmount = regGroup(formDate, 'amount');
               if (!isAmount) return ElMessage.error("支付金额不能为空");
         ...
         //组合支付不为空
         const regGroup = (formDate, type) => {
            let { orderPaymentLogList, paymentType } = formDate;
            if (paymentType == 2) {
               let isRequired = orderPaymentLogList.every(item => item.amount);
               let isChannel = orderPaymentLogList.every(item => item.channel);
               if (type == 'amount') { return isRequired } else { return isChannel }
            } else {
               return true;
            }
         };
         ...
         const processingData = (formDate) => {
            ...
            payAppendixes = payAppendixes ? (payAppendixes = JSON.stringify(payAppendixes)) : null;
         ...
         const tableStr = (val) => {
            let str = `
    <ol class="tableUl">
       ${val.payments.map(item => {
               return `  <li>
         支付方式：${item.channel_name} &emsp;支付金额：${item.amount} &emsp;交易号：${item.trade_no || '无'}
        </li> `;
            }).join("")}
         <p>付款时间：${val.payment_time}</p> 
    </ol>
         `;
            return str;
         };
